...
In nearly all HTTP frameworks for Node.js, routing is implemented by scanning a list of regular expressions until one is matched. This means routing is performed in linear time proportionate to the number of routes needed by the application. For small services that only have ten or twenty different routes, this is effectively a non-issue. However, it's not abnormal for very large services to contain close to one-hundred routes at a time, raising a legitimate concern.
