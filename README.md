...
Node.js is heavily asynchronous and event loop driven. Asynchronous programming is especially relevant within the context of an HTTP service, since many connections must be managed concurrently. In [section 1.2](#12-plugin-system) we did a cursory overview of common asynchronous strategies. In particular, we made a distinction between low-level *callbacks* and high-level *promises*. However, different strategies exist for solving different problems. Promises and callbacks are typically designed to solve the problem of representing a *single* future event. On the other hand, Node.js also has *[event emitters](https://nodejs.org/api/events.html)* and *[streams](https://nodejs.org/api/stream.html)* which are both designed to solve the problem of representing *multiple* future events. An analogy in synchronous programming is the distinction between a *single value* and an *iterator* or *array*.
