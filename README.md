...
Unlike in Vapr, routing in Koa is performed by linearly scanning a list of [regular expressions](https://en.wikipedia.org/wiki/Regular_expression) until a match is found. Because regular expressions are used internally, Koa allows programmers to create routes from regular expressions directly too (they are not limited to string pathnames). Routing in Vapr is not implemented with regular expressions for reasons described in sections [1.4](#14-routing) and [4.2](#42-scaling-a-router), and so routing based on regular expression is not supported.
